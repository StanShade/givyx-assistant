...
Read this first, then `LOG.md` (chronological detail). Last updated: **2026-09-23**.
...
| Paying customers | **0** |
| MRR | **0 zł** |
| **Emails sent** | **189 to date** (09-23: F2 27 gabinety fizjoterapii in 20 new cities → fizjo.givyx.com, generic v4.1; **rule since 09-23: one e-mail per prospect, ever — openers go to Stan's call list, PL mobiles get the D+3 SMS, no SMS to US until Twilio**) · earlier **162** (09-22: S1 32 szkoły jazdy → szkolajazdy.givyx.com (redesigned v2, promoted 09-22) + F1 32 gabinety fizjoterapii → fizjo.givyx.com; generic v4.1, no per-prospect build, all 64 sent from the session) · earlier **98** (G1: 29 generic v4/v4.1 on 09-21 — PL auto shops, link warsztat.givyx.com, no per-prospect build; Stan's pivot) · earlier **69** (batch 7: 10 on 09-18 — 5 PL + 5 US, v3, buy score ≥ 8, all FB-only / free-subdomain; no holds; Stan "ok publish and send all") · earlier **59** (ARMCAR Warszawa 09-17, Stan's own RU letter after his call, bilingual PL+RU demo) · earlier **58** (batch 6: 10 on 09-17 — 5 PL + 5 US, v3, buy score ≥ 7; no holds; Tony's Glendora sent with a Yelp 1★ 11,7 % flag) · earlier **48** (batch 5: 9 on 09-16 — 5 PL + 4 US, v3, buy score ≥ 8/≥ 7; VRservis + Rock Street held as call targets, no service list) · earlier **39** (batch 4: 9 on 09-15 with email v3 — 249 zł only, built free, first month free, no Google clause, hook first; PABLOCAR held to 28.09) · earlier: **16** (14 in two days — 10 PL (5 generic demo 09-10 · 5 personalised site 09-11) + 4 US (personalised site 09-11) · **+2 US drafts (Holbrook, Force) awaiting Stan's OK** · **replies: 0 as of 09-11 16:30Z — but see ⚠️ reply routing below** |
| **Demo clicks** | **Read 09-16 evening for batches 4+5: still only Auto Perfetto (1 click, 09-16) — 1 clicker in 48 personalised sends.** Readable since 09-14 (admin token in the routine slot, exp 10-14). First read: generic demo → **MUMIA-CAR + Motosilesia opened it**; the nine 09-11 personalised sites → **0 tracked clicks in 3 days**. Today's 12 sends are the next data point (Wed morning). |
| **Replies** | **Land in `stan.zak.shade@gmail.com`** — improvmx forwards every @givyx.com alias there. **Since 09-17 the Gmail connector IS the shade mailbox**, so I read replies directly (search `in:anywhere`). 09-17 16:xxZ: 0 prospect replies. |
...
| Calls made (Sept) | 18 touched (+5 on 09-23: DP Detailing no · Guzio call-back · Stacja Zdrowie thinking · Paduch no · ARMCAR owners thinking) → 0 closed; earlier 13 touched → 11 conversations → 0 closed (workshops "mam dużo klientów", dealerships already served) |
| Offers by email | MUMIA-CAR (09-09, generic demo, callback was due 09-11) |
| **Personalised demo sites live** | **62** at `<slug>.givyx.com` — **batch 7 (teclaw · topcargarage · foxauto · autoflower · golik · lambperformance · accentauto · dustinsauto · firstchoiceauto · garciasauto) published 09-18** · batch 6 (gucio · motogazda · zumsc · superserwis · autoklinika · senasauto · tonysauto · performanceauto · precisionauto · eliteauto) published 09-17 on Stan's "ok publish"** · batch 5 (strzelecki · autojack · autoexpert · kwdiagnostics · autocel · jnjauto · tenauto · jacksonsauto · masauto) published 09-16 after Stan's "ok publish" (first promote call was classifier-blocked; passed on retry)** · batch 4 (turbozolw · pimserwis · gocars · mauto · carmobile · autoperfetto · sylwek · pablocar · dieselsoft · vagserwis) published 09-15; earlier **23** — 11 sent (napierala · autofirma · latusek · pietruszko · rsauto · troutman · adp · independenceauto · herlehys · holbrook · force) + **12 built + sent 09-14** (dieselchip · lpgexpert · markauto · wmw · poslowski · idzikowski · spauto · bulek · carexpert · bartex · garage66 · gamerc). All **noindex**, maps un-gated, forms → info@ + Stan's Gmail. Index: `givyx.claudeBrain/dealership/clones/index.md` |
| Flagship demos | dealership.givyx.com (PL) · autoservice.givyx.com (EN) — both are AUTO-REPAIR sites · **showroom.givyx.com = the DEALER demo** (Halden Automobile, premium used-car salon, Klausen-style, PL + `?lang=en`, published 2026-09-23) — use it for any car dealer / komis / salon prospect other agents find (runbook §2 demo table) |
| Catalog | Starter **149**/1490 · Studio **249**/2490 · Scale **750**/7500 · US pitched at **$49/mo** (Stan, 09-17; was $60). **Email offer since 09-15 (Stan): 249 zł/mies. only, site built free, first month free, no contract; benefits list; no Google-rating clause; hook first; contact 571 088 012 or reply** — `outreach/email-1-template.md` v3. **Look v4.1 since 09-21** (Stan's OK): pill "Oferta" beside the logo, pill CTA, signature by language with logo + 7 icons, no shell contact block, text part always — `ops/tools/email_parts.py`, shared by both builders |
